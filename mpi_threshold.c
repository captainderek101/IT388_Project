 ...
 * Compile: mpicc -o pngThreshold mpi_threshold -lpng
 * 
 * Execute: mpiexec -n <numcores> ./pngThreshold <image.png> <outimage.png>
 */
...
int main(int argc, char *argv[]) {
   ...
   if (argc < 3) {
       if (rank == 0) {
           fprintf(stderr, "USAGE: mpiexec -n <number of cores> ./pngtest <input_filename> <output_filename>\n");
       }
       MPI_Abort(comm, 1);
   }
